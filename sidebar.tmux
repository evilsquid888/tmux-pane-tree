...
set-hook -g window-pane-changed[201] "run-shell -b '#{d:current_file}/scripts/ensure-sidebar-pane.sh #{pane_id} #{window_id}'"
set-hook -g client-focus-in[202] "run-shell -b '#{d:current_file}/scripts/ensure-sidebar-pane.sh #{pane_id} #{window_id}'"
set-hook -g after-select-window[203] "run-shell -b '#{d:current_file}/scripts/ensure-sidebar-pane.sh #{pane_id} #{window_id}'"
set-hook -g client-focus-in[204] "run-shell -b '#{d:current_file}/scripts/remember-main-pane.sh #{pane_id}'"
set-hook -g client-focus-in[205] "run-shell -b '#{d:current_file}/scripts/clear-pane-state.sh #{pane_id}'"
...
set-hook -g session-window-changed[215] "run-shell -b '#{d:current_file}/scripts/ensure-sidebar-pane.sh #{pane_id} #{window_id}'"
